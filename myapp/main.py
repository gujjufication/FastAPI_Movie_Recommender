...
def add_movie(movie_title:str, category: str, release_date: str, user_rating: float, db: Session = Depends(get_db)):
    ...
    # Normalize title to lowercase for consistent comparisons
    normalized_title = movie_title.lower()
    existing_movie = db.query(Movie).filter(Movie.title == normalized_title).first()
    if existing_movie:
    ...
    # Reload data to include the newly added movie in future recommendations
    global ratings, movies, similarity_df
    ratings, movies = recommender.load_data()
    similarity_df = recommender.create_similarity_matrix(ratings)
